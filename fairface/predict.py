...
def predict_age_gender_race(save_prediction_at, imgs_path = 'cropped_faces/'):
    ...
    for index, img_name in enumerate(img_names):
        ...
        precision = 3
        race_score = np.round(np.exp(race_outputs) / np.sum(np.exp(race_outputs)), precision)
        gender_score = np.round(np.exp(gender_outputs) / np.sum(np.exp(gender_outputs)), precision)
        age_score = np.round(np.exp(age_outputs) / np.sum(np.exp(age_outputs)), precision)
...
if __name__ == "__main__":
    ...
    print(f"detected faces are saved at {save_detected_at}")
    # Please change test_outputs.csv to actual name of output csv. 
    predict_age_gender_race(output_csv, save_detected_at)
