...
""" 
    ...
    RACE:
    either a description standing before or after GENDER & AGE (concrete description: see below)
...
# build diverse prompts for the inserted image_numbers based on the attributes available for FairFace images if random_race_desc is True, 
# otherwise the descriptors per race are static
# returns dict: (image_number: prompt)
def get_ff_prompts(image_numbers, attr_path, race_indices=None, random_race_desc=True):
    attributes = pd.read_csv(attr_path)
    prompts = {}
    for i, img_nmb in enumerate(image_numbers):
        current = attributes.iloc[img_nmb-1]
        
        # check whether a race is passed
        race_index = race_indices[i] if race_indices is not None else None
        
        # if a race is passed, use this, otherwise use the annonated race of the image
        current_race = races[race_index] if race_index is not None else current["Race"]
        
        # build a prompt for the given image
