...
def running_tests(data_path, dataset_name, output_dir, weights_dir, csv_file):

    print("CURRENT OUT FOLDER")
    print(output_dir)
    datasets ={dataset_name: os.path.join(data_path, dataset_name)}
    # datasets ={'latent-diffusion_noise2image_FFHQ': './TestSet/latent-diffusion_noise2image_FFHQ/'}
    # datasets = {os.path.basename(os.path.dirname(_)): _ for _ in glob.glob(DATA_PATH+"*/")}# {os.path.basename(os.path.dirname(ds)): ds} #
...
# parser.add_argument("--data_dir", type=str, help="The path to the images of the testset on which the operations"+
#                     " have been pre applied with the provided code", default="../CelebA/")
# parser.add_argument("--out_dir", type=str, help="The Path where the csv containing the outputs of the networks"+
#                     " should be saved", default="./results_tst")
